...
export default function SalesPage() {
  ...
  return (
    <AdminShell>
      <PageHeader
        eyebrow="Reports"
        title={`Sales — ${rangeLabel(dateRange)}`}
        actions={
          <>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
        <KpiTile label="Revenue" value={`$${(data?.totalRevenue ?? 0).toFixed(2)}`} />
        <KpiTile label="Orders" value={String(data?.orderCount ?? 0)} />
        <KpiTile label="Avg. order value" value={`$${(data?.averageOrderValue ?? 0).toFixed(2)}`} />
      </div>

      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <ChartWrapper title="Revenue by day">
          <SimpleLineChart data={byDayChart} valueFormatter={(value) => `$${value.toFixed(0)}`} />
        </ChartWrapper>

        <ChartWrapper title="By payment method">
          <SimplePieChart data={byMethodChart} valueFormatter={(value) => `$${value.toFixed(0)}`} />
        </ChartWrapper>
      </div>
    </AdminShell>
  );
